...
public class Technology {
	...
	@ZenMethod
	public static void addTechnology(String page, String prev, IItemStack icon, int x, int y, String name,
			IIngredient[] item) {
		addTechnology(page, prev, null, icon, false, x, y, name, item);
	}

	@ZenMethod
	public static void addTechnology(String page, String prev, IItemStack icon, boolean hide, int x, int y, String name,
			IIngredient[] item) {
		addTechnology(page, prev, null, icon, hide, x, y, name, item);
	}

	@ZenMethod
	public static void addTechnology(String page, String prev, String[] secret, IItemStack icon, int x, int y,
			String name, IIngredient[] item) {
		addTechnology(page, prev, secret, icon, false, x, y, name, item);
	}

	@ZenMethod
	public static void addTechnology(String page, String prev, String[] secret, IItemStack icon, boolean hide, int x,
			int y, String name, IIngredient[] item) {
		ftgumod.technology.Technology p = null;
		if (prev != null) {
			p = TechnologyHandler.getTechnology(prev);
			if (p == null) {
				MineTweakerAPI.logWarning("[" + FTGUTweaker.name + "] No " + Technology.name + " found for " + prev
						+ ". Command ignored!");
				return;
			}
		}
		...
		if (secret != null) {
			...
			for (String s : secret) {
				...
				if (t != null)
					...
					MineTweakerAPI.logWarning("[" + FTGUTweaker.name + "] No " + Technology.name + " found for " + s
							+ " in " + Technology.name + " " + p.getLocalisedName() + ". Secret unlock ignored!");
			}
			...
			MineTweakerAPI.apply(new AddTech(new ftgumod.technology.Technology(PAGE.get(page), p, ls2,
					InputHelper.getStack(icon), hide, x, y, name, InputHelper.toObjects(item))));
		} else {
			MineTweakerAPI.apply(new AddTech(new ftgumod.technology.Technology(PAGE.get(page), p, null,
					InputHelper.getStack(icon), hide, x, y, name, InputHelper.toObjects(item))));
		}
	}
